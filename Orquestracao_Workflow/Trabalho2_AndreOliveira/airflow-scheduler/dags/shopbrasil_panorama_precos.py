...
Trabalho 2 (CeleryExecutor):
  Todas as tasks usam a fila Celery "shopbrasil" (default_args.queue).
  O scheduler apenas enfileira no Redis; os workers (celery worker -q shopbrasil)
  consomem e executam. Cada task registra worker=<hostname> nos logs.
...
@dag(
    ...
    tags=["shopbrasil", "pricing", "etl", "fakestore", "celery"],
    doc_md=__doc__,
    max_active_runs=1,
)
def shopbrasil_panorama_precos():
    ...
    def ingestao():
        ...
        def buscar_produtos() -> list[ProdutoResumo]:
            ...
            log.info(
                "Buscando produtos na FakeStore API: %s | worker=%s",
                FAKESTORE_PRODUCTS_URL,
                worker,
            )
        ...
        def validar_produtos(produtos: list[ProdutoResumo]) -> list[ProdutoResumo]:
            ...
            log.info(
                "Iniciando validação de %d produtos | worker=%s",
                len(produtos),
                socket.gethostname(),
            )
        ...
        def listar_categorias(produtos: list[ProdutoResumo]) -> list[str]:
            ...
            log.info(
                "Listando categorias a partir de %d produtos | worker=%s",
                len(produtos),
                socket.gethostname(),
            )
    ...
    def analise(
        ...
        def calcular_metricas(
            ...
            log.info(
                "Calculando métricas — categoria=%s | pool=%s | worker=%s",
                categoria,
                ECOMMERCE_POOL,
                socket.gethostname(),
            )
        ...
        def consolidar(metricas: list[MetricaCategoria]) -> list[MetricaCategoria]:
            ...
            log.info(
                "Métricas consolidadas: %s | worker=%s",
                [m["categoria"] for m in consolidado],
                socket.gethostname(),
            )
            return consolidado
        ...
        def carregar(registros: list[MetricaCategoria], **context) -> int:
            ...
            log.info(
                "Gravando %d registros — connection=%s | data=%s | worker=%s",
                len(registros),
                POSTGRES_CONN_ID,
                data_referencia,
                socket.gethostname(),
        ...
        def verificar(total_inserido: int, **context) -> None:
            ...
            log.info(
                "Verificando carga — total=%d | worker=%s",
                total_inserido,
                socket.gethostname(),
            )
